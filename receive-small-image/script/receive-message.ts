...
createToken(version, `丢图收消息 ${version}`).then((data: string) => {
...
const printResult = async () => {
  ...
  for (const r in messageResult) {
    // 120 is the TOTAL / (contactlist.length + roomlist.length)
    reportData.push([r, 120, messageResult[r].length, messageResult[r].length === 120 ? '正常' : '异常'])
  }
